...
class GoogleWeatherInput(BaseModel):
...
class GoogleNationalWeatherAPI(BaseTool):
    ...
    args_schema: Type[BaseModel] = GoogleWeatherInput
    
    gmaps: Any
    google_api_key: Optional[str] = None
    
    @root_validator(pre=True)
    # Check the following link for updating the root_validator decorator; pull from pydantic v2, instead of langchain/pydantic v1.
    ...
    def _geocode(self,
                location: str = Field(description="The stated town or city, with the associated state, if stated")
                ) -> dict:
        import googlemaps
    ...
    def _return_temperature(self,
        ...
        forecast_hourly = requests.get(weather.json().get('properties').get('forecastHourly'))
